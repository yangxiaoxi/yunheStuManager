...
%>
<div class="box">
    <form id="uploadForm" action="roleServlet?method=update&role_id=<%=role_id %>" method="post" class="form-horizontal" role="form">
        <div class="form-group">
            <label for="user_id" class="col-sm-2 control-label">角色ID:</label>
            <div class="col-sm-10">
                <input type="text" class="form-control" name="class_id" 
                id="class_id" disabled="disabled" value="<%=role_id %>" />
            </div>
        </div>
        <div class="form-group">
        ...
        <div class="form-group">
            ...
            List<Integer> myPermission = pManager.queryByRole_id(Integer.parseInt(role_id));
            for(Permission  p:listPermission){
            	String select = "";
            	if(myPermission.contains(p.getPermissin_id())){
            		select="checked='checked'";
            	}
            %>
            <input  type="checkbox" name="permission" value="<%=p.getPermissin_id() %>" <%=select %>/><%=p.getPermission_name() %>&nbsp;&nbsp;
            <%
            }
            %>
        </div>
